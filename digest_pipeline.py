...
class AIEnrichmentService:
    def __init__(
        ...
        *,
        ...
        fetch_article_text_func: Callable[..., tuple[str, str]] | None,
    ...
    def apply_ai_importance(self, items: list[Item]) -> None:
        ...
        candidates = sorted(items, key=lambda x: x["score"], reverse=True)[: self._ai_importance_max_items]
        ...
        for idx, item in enumerate(candidates, start=1):
            if idx == 1 or idx % 5 == 0 or idx == total:
                self._log(f"AI 중요도 진행: {idx}/{total}")
            if self._article_fetch_enabled and self._fetch_article_text and fetch_budget > 0:
                full_text = item.get("fullText") or ""
                if len(full_text) < self._article_fetch_min_chars:
                    text, resolved_url = self._fetch_article_text(
                        item.get("link") or "",
...
class DigestPipeline:
    ...
    def fetch_grouped_and_top(
        ...
        for source_idx, source in enumerate(sources, start=1):
            ...
            for entry_idx, entry in enumerate(feed.entries[: self._max_entries_per_feed], start=1):
                if entry_idx == 1 or entry_idx % 20 == 0 or entry_idx == total_entries:
                    self._log(f"항목 처리: {topic} {entry_idx}/{total_entries}")
        ...
        return grouped_items, self.pick_top_with_mix(all_items, top_limit)
